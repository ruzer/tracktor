<script lang="ts">
  ...
  async function handleDelete(logId: number) {
    ...
    try {
      const response = await fetch(
        `${env.PUBLIC_API_BASE_URL}/api/maintenance-logs/${logId}`,
        {
          method: "DELETE",
          headers: {
            "X-User-PIN": browser ? localStorage.getItem("userPin") || "" : "",
...
<div class="mt-8">
  ...
  {:else}
    <div class="overflow-x-auto">
      <table
        class="min-w-full rounded border border-gray-200 shadow-sm dark:border-gray-700"
      >
        <thead class="bg-gray-100 dark:bg-gray-800">
          <tr>
            <th
              class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-200"
              >Date</th
            >
            <th
              class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-200"
