<script lang="ts">
	...
	async function load() {
		...
		error = '';
		try {
			const res = await fetch(getApiUrl(`/api/vehicles/${vehicleId}/plates`), {
	...
	async function add() {
		try {
			...
			if (res.ok) {
				form = { plate: '', issuedDate: '', reason: '', isCurrent: true };
				await load();
			} else error = (await res.json()).message || $t('errors.fetchFailed');
		} catch {
			error = $t('errors.networkError');
	...
	async function markCurrent(id: string) {
		await fetch(getApiUrl(`/api/vehicles/${vehicleId}/plates/${id}/current`), {
			method: 'PATCH',
			headers: { 'X-User-PIN': localStorage.getItem('userPin') || '' }
		});
		await load();
	...
	async function retire(id: string) {
		await fetch(getApiUrl(`/api/vehicles/${vehicleId}/plates/${id}/retire`), {
			method: 'PATCH',
			headers: {
				'Content-Type': 'application/json',
				'X-User-PIN': localStorage.getItem('userPin') || ''
			},
			body: JSON.stringify({ retiredDate: new Date().toISOString().slice(0, 10) })
		});
		await load();
	}
</script>
	...
	<div class="mb-4 rounded bg-white p-4 shadow dark:bg-gray-800">
		<div class="grid grid-cols-4 gap-4">
			<input
				...
				placeholder={$t('forms.labels.plate')}
				bind:value={form.plate}
			/>
			<input
				type="date"
				class="rounded border p-2 dark:bg-gray-700"
				placeholder={$t('forms.labels.issueDate')}
				bind:value={form.issuedDate}
			/>
			<input
				class="rounded border p-2 dark:bg-gray-700"
				placeholder={$t('forms.labels.reason')}
			...
			<thead class="bg-gray-200 dark:bg-gray-700">
				<tr>
					<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-200"
						>{$t('forms.labels.plate')}</th
					>
					<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-200"
						>{$t('forms.labels.issueDate')}</th
					>
					<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-200"
						>{$t('forms.labels.retireDate')}</th
					>
					<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-200"
						>{$t('forms.labels.reason')}</th
					>
					<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-200"
			...
			<tbody>
				{#each rows as r (r.id)}
					<tr class="border-b border-gray-200 last:border-b-0 dark:border-gray-700">
						...
						<td class="flex gap-2 px-4 py-2 text-gray-900 dark:text-gray-100">
							<Button
								type="button"
								variant="hero"
								text={$t('forms.labels.current')}
								onclick={() => markCurrent(r.id)}
							/>
							<Button
								type="button"
								variant="secondary"
								text={$t('forms.labels.retire')}
								onclick={() => retire(r.id)}
							/>
						</td>
					</tr>
