...
export const translations = derived(
	...
	([$availableLanguages, $currentLanguage]) => {
		return (
			$availableLanguages[$currentLanguage]?.translation ||
...
export const languageOptions = derived(availableLanguages, ($availableLanguages) => {
...
export async function initializeI18n() {
	try {
		...
		if (browser) {
			...
			const languageToUse =
				saved && languages[saved] ? saved : languages[browser_lang] ? browser_lang : 'en';
